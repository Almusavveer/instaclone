...
const ProfileCard = ({ user }) => {
  ...
  useEffect(() => {
    const fetchFollowData = async () => {
      try {
        const res = await axios.get(`${API_BASE}/api/post/getallfolloweringcount`, {
  ...
  //       { userId: user.id }, // adjust based on your backend
  //       { withCredentials: true }
  //     );
      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-500 text-sm">@{user.username || user.name}</p>
          </div>
        </div>
        ...
        <div className="mt-4">
          <p className="text-gray-600 leading-relaxed">
            focuses on{" "}
            <span className="font-medium text-gray-800">simplicity</span> &{" "}
            <span className="font-medium text-gray-800">usability</span>.
          </p>
        </div>
        ...
        {/* <p className="text-xs text-center text-gray-400 mt-4">
          {isFollowing ? "You are following Noah" : "Click to follow Noah"}
        </p> */}
      </div>
    </div>
  );
